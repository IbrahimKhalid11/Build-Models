...
public:
    ...
    vector<vector<int>> detectOutliersIQR(vector<vector<double>>& data) {
        ...
        if (data.empty() || data[0].empty()) return outliers;
    ...
    //vector<vector<double>> standardScaler(vector<vector<double>>& data) {
    ...
    //            }
    //            else {
    //                scaledData[i][j] = data[i][j] - mean[j];
    //            }
    //        }
    //    }
    ...
    pair<int, int> shape(const DataFrame& df) {
        if (df.empty())
            return { 0, 0 };
        return { df.size() - 1, df[0].size() }; // Exclude the header row
    }

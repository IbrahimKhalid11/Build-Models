...
public:
    ...
    void dropColumnsFromIndices(std::vector<std::vector<std::string>>& data, const std::vector<int>& dropIndices) {
        // Sort the indices in descending order to avoid shifting issues
        std::vector<int> sortedIndices = dropIndices;
        std::sort(sortedIndices.rbegin(), sortedIndices.rend());

        for (int index : sortedIndices) {
            // Check if the index is valid
            if (index >= 0 && !data.empty() && index < data[0].size()) {
                for (auto& row : data) {
                    row.erase(row.begin() + index);
                }
            }
            else {
                std::cout << "Invalid column index: " << index << std::endl;
            }
        }
    }
    ...
    // Function to encode a specified column with unique numbers
    std::vector<std::vector<std::string>> encodeColumnWithNumbers(std::vector<std::vector<std::string>>& data, const std::string& columnName) {
        if (data.empty()) {
            std::cerr << "The DataFrame is empty." << std::endl;
            return data;
        }

        // Get the headers and find the index of the specified column
        ...
        // Create a map to store unique values and their encoded numbers
        std::unordered_map<std::string, int> uniqueValues;
        int uniqueCount = 0;

        // Iterate through the rows to assign unique numbers to each value
        for (int i = 1; i < data.size(); ++i) {
            const std::string& value = data[i][columnIndex];
            if (uniqueValues.find(value) == uniqueValues.end()) {
                uniqueValues[value] = uniqueCount++;
            }
        }

        // Modify the DataFrame with encoded numbers
        for (int i = 1; i < data.size(); ++i) {
            data[i][columnIndex] = std::to_string(uniqueValues[data[i][columnIndex]]);
        }

        return data;
    }
    std::vector<std::vector<std::string>> selectColumns(const std::vector<std::vector<std::string>>& data, const std::vector<std::string>& columnNames) {
        if (data.empty()) {
            std::cerr << "The DataFrame is empty." << std::endl;
            return data;
        }
        ...
        for (const auto& columnName : columnNames) {
            ...
            if (columnIndex != -1) {
                ...
                std::cerr << "Column name " << columnName << " not found." << std::endl;
            }
        }
        ...
        for (int index : columnIndices) {
            newHeaders.push_back(headers[index]);
        }
